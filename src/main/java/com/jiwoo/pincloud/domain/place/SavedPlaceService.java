...
public class SavedPlaceService {
    ...
    public SavedPlaceResponse create(Long userId, SavedPlaceCreateRequest request) {
        ...
        SourceLink sourceLink = sourceLinkRepository.save(
                SourceLink.builder()
    ...
    public SavedPlaceResponse findById(Long id, Long userId) {
        ...
        SourceLink sourceLink = sourceLinkRepository.findBySavedPlaceId(savedPlace.getId()).orElse(null);
        return toResponse(savedPlace, sourceLink);
    }
    ...
    public List<SavedPlaceResponse> findByBbox(
        ...
        List<SavedPlace> savedPlaces = savedPlaceRepository.findByUserIdAndBbox(userId, minLat, maxLat, minLng, maxLng)
                .stream()
    ...
    public void delete(Long id, Long userId) {
        SavedPlace savedPlace = getSavedPlace(id, userId);
        sourceLinkRepository.findBySavedPlaceId(savedPlace.getId()).ifPresent(sourceLinkRepository::delete);
    ...
    private SavedPlace getSavedPlace(Long id, Long userId) {
        return savedPlaceRepository.findByIdAndUserId(id, userId)
    ...
    private Platform resolvePlatform(SavedPlaceCreateRequest request) {
        return request.getPlatform() != null ? request.getPlatform() : Platform.detect(request.getOriginalUrl());
    }
    ...
    private List<SavedPlaceResponse> toResponses(List<SavedPlace> savedPlaces) {
        ...
        Map<Long, SourceLink> sourceLinkMap = sourceLinkRepository.findAllBySavedPlaceIdIn(
                        savedPlaces.stream().map(SavedPlace::getId).toList()
                ).stream()
                .collect(Collectors.toMap(sourceLink -> sourceLink.getSavedPlace().getId(), Function.identity()));
