...
class SavedPlaceControllerTest {
  ...
  @Autowired private SourceLinkRepository sourceLinkRepository;

  @MockitoBean private KakaoAuthClient kakaoAuthClient;

  @MockitoBean private KakaoPlaceClient kakaoPlaceClient;
  ...
  void GET_savedPlaces_id_200_OK를_반환한다() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/saved-places/{id}", savedPlace.getId())
                .header(HttpHeaders.AUTHORIZATION, 인증_헤더(user)))
  ...
  void PATCH_savedPlaces_id_200_OK를_반환한다() throws Exception {
    ...
    mockMvc
        .perform(
            patch("/api/saved-places/{id}", savedPlace.getId())
                .header(HttpHeaders.AUTHORIZATION, 인증_헤더(user))
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
        .andExpect(status().isOk())
  ...
  void DELETE_savedPlaces_id_204_No_Content를_반환한다() throws Exception {
    ...
    mockMvc
        .perform(
            delete("/api/saved-places/{id}", savedPlace.getId())
                .header(HttpHeaders.AUTHORIZATION, 인증_헤더(user)))
  ...
  void GET_savedPlaces_id_인증이_없으면_403_Forbidden을_반환한다() throws Exception {
    ...
    // then
    mockMvc
        .perform(get("/api/saved-places/{id}", savedPlace.getId()))
